...
import groupIcon from '../../assets/Images/Group 11889-1.svg';
import arrowIcon from '../../assets/Images/down-arrow-1.svg';
import businessFinanceIcon from '../../assets/Images/business-and-finance.svg';
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center">
            <img src={groupIcon} alt="Recent" className="w-5 h-5 md:w-6 md:h-6" />
          </div>
...
function TransactionItem({ transaction }: { transaction: Transaction }) {
  const getTransactionMeta = (category: string) => {
    const map: Record<string, { src: any; bg: string }> = {
      Refund: { src: fileStorageIcon, bg: 'bg-blue-50' },
      Travel: { src: flightsIcon, bg: 'bg-emerald-50' },
      Shopping: { src: megaphoneIcon, bg: 'bg-pink-50' },
      Charged: { src: businessFinanceIcon, bg: 'bg-blue-50' },
      Charge: { src: businessFinanceIcon, bg: 'bg-blue-50' },
    };
        ...
        {transaction.description && (
          <div className="flex items-center gap-2 md:gap-3 mt-1.5 md:mt-2">
            <div className="w-6 h-6 md:w-8 md:h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#325BAF' }}>
              <img src={businessFinanceIcon} alt="desc" className="w-3 h-3 md:w-3.5 md:h-3.5" style={{ filter: 'brightness(0) invert(1)' }} />
            </div>
